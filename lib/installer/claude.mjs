 ...
 * @param {string} ctx.rootDir       Where @reydo/devline is installed
...
export async function installClaude(ctx) {
  ...
  // 5. Write allowedTools to settings.json
  const settingsPath = join(claudeDir, 'settings.json');
  writeClaudeAllowedTools({ settingsPath, rootDir, dryRun, log });
}

/**
 * Merge devline allowedTools entries into ~/.claude/settings.json.
 */
function writeClaudeAllowedTools({ settingsPath, rootDir, dryRun, log }) {
  const bins = ['dl-init', 'dl-feature', 'dl-fix', 'dl-review', 'dl-plan', 'dl-sync',
                 'dl-verify', 'dl-check', 'dl-explain', 'dl-workspace', 'dl-benchmark',
                 'dl-test', 'dl-log', 'dl-update', 'dl-install', 'dl'];
  const entries = [
    `Read(${rootDir}/**)`,
    ...bins.map(b => `Bash(${b}*)`),
  ];
  ...
  if (dryRun) {
    log.info(`Would add ${entries.length} allowedTools entries to ${settingsPath}`);
    return;
  }
  ...
  if (toAdd.length === 0) {
    log.info('allowedTools already up to date');
    return;
  }
  settings.allowedTools = [...existing, ...toAdd];
  writeJsonFile(settingsPath, settings);
  log.action(`Added ${toAdd.length} allowedTools entries to ${settingsPath}`);
}
...
function buildSkillTable(skillsDir) {
  const skills = [
    ...
    ['dl-benchmark', 'dl-benchmark/SKILL.md', 'Run devline benchmarks'],
    ['dl-explain',   'dl-explain/SKILL.md',   'Explain business logic / code'],
  ];
