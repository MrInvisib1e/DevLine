...
**Invoked as:** `/df-review [--base <branch>] [--headless]`
...
1. Check `.devflow/` exists — if not: run in degraded mode (skip convention checks)
...
2. Read full diff: `git diff <base>...HEAD`
   - If >30 files: read top 30 by node connectivity (run `df-explain --rank --budget 30` and cross-reference with changed files)
3. For each changed file: `df-explain --node <file>` → collect symbol names + inbound/outbound edges
...
Read `review_checks` array from `.devflow/config.json`. Each object entry with `name`, `severity`, `convention` fields is applied as an additional check. Only flag if diff matches the pattern described in `convention`.
...
If `.devflow/` missing (CI without init):
- Skip convention checks
- Provide general observations only (file-level, no graph context)
- T2 Inform: "Running in degraded mode — no DevFlow memory. Convention checks skipped."
...
- Output written to `devflow-headless.log`
