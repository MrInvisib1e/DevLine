...
A/B test any DevFlow skill against falsifiable assertions to measure its effectiveness.
...
Step 2: Record a scenario (or pick from .devflow/benchmark/scenarios/)
...
Assertions go in: `.devflow/benchmark/<benchmark-name>/assertions.json`
...
Format: `.devflow/benchmark/<benchmark-name>/scenario.md`
...
```bash
df-benchmark run --scenario <name> --mode control
```
...
```bash
df-benchmark run --scenario <name> --mode treatment --skill <skill-name>
```
...
Save to: `.devflow/benchmark/<benchmark-name>/results/<timestamp>.json`

---

## Running df-benchmark

```bash
# Run a full benchmark
df-benchmark run --scenario tdd-enforcement --skill tdd

# List all saved benchmarks
df-benchmark list

# Show last result for a benchmark
df-benchmark show tdd-enforcement

# Re-run last benchmark with updated skill
df-benchmark rerun tdd-enforcement
```
...
Pre-written scenarios in `.devflow/benchmark/scenarios/`:
...
`df-benchmark` is a **stub** in v4.0. The `df-benchmark` CLI script exists and accepts the commands above, but scenario replay is manual — the grading step requires the human to assess the agent log.
