...
```bash
readlink .devline/active
```

If `.devline/active` does not exist: error E10 — "No active feature found. Use `/feature <description>` to start one."

If it exists but the target directory is missing: try to find the plan by listing `.devline/plans/` and ask user which to resume.
...
CHECKPOINT: "[Devline] Resuming at: Phase <N>, Slice <name if applicable>"
...
- `.devline/active` symlink points to non-existent directory: list `.devline/plans/` and ask user which to resume
...
> For quick mode reference, error codes (E01–E15), guard rails, and abort cleanup protocol, see [skills/dl-feature/SKILL.md](../SKILL.md).
