...
**Invoked as:** `/df-plan <task>`, `/df-plan --brainstorm <idea>`, `/df-plan --quick <task>`
...
| `/df-plan --brainstorm <idea>` (explicit) | Force brainstorming mode |
| `/df-plan --quick <task>` (explicit) | Skip brainstorming AND research tiers → go straight to Phase 3: Plan |
...
| Multi-module, known pattern, some uncertainty | Standard | + `df-explain <relevant symbols>` + read existing docs |
...
- Agent A: Search codebase for similar patterns (`df-explain --rank`, `df-explain <component>`)
- Agent B: Read relevant external docs/references (API docs, schema files, config)
- Agent C: Check `FILE_CHANGES_WITH` edges for hidden coupling (`df-explain --node` on affected files)

**Co-change coupling:** Always run `df-explain --node <file>` on affected files to check which files historically change together. Files with `FILE_CHANGES_WITH` edges are likely coupled — include them in the plan.
...
Run `df-explain <component>` for each module affected. Identify reference feature from memory.md — a similar existing feature to use as a pattern.
...
After approval: T2 Inform — "Plan saved to `.devflow/plans/{path}/plan.md`. Use `/df-feature` to execute."
