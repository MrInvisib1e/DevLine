...
**Invoked as:** `/df-verify` (call before any completion claim)
...
|----------|----------|-------------|
| Slice done | `test_cmd` from `.devflow/config.json` | All tests pass, exit 0 |
...
| Memory synced | `/df-sync` | `last_synced == HEAD` in config.json |
| Feature complete | `test_cmd && build_cmd` | Both exit 0 |
| Plan tasks done | Re-read `.devflow/plans/.../plan.md` | Each task checkbox ticked |
