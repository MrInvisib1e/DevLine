...
Generated by `df-init`, read by all skills. Located at `.devflow/config.json`.
...
| `review_checks` | array | Which checks `/df-review` runs; customizable |
| `auto_skills` | array | Skill auto-activation rules (regex trigger → skill name) |
| `quality_hooks` | object | Commands for `df-check` (lint/format/typecheck) |

### Modes

| Mode | Created by | `.devflow/` location | Memory |
|------|-----------|---------------------|--------|
| `project` | `df-init` (default) | git root | Single `memory.md` |
| `orchestrator` | `df-init --orchestrator` | parent dir | Aggregates child `memory.md` files |
