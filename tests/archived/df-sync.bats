...
setup() {
  ...
  export FIXTURE_AI="$BATS_TEST_DIRNAME/fixtures/ai-responses/dl-sync-response.json"
  export DF_SYNC="$BATS_TEST_DIRNAME/../bin/dl-sync"
  export DF_INIT="$BATS_TEST_DIRNAME/../bin/dl-init"
  export PATH="$BATS_TEST_DIRNAME/../bin:$PATH"
  export DEVLINE_AI_MOCK=1
  export DEVLINE_AI_MOCK_FILE="$FIXTURE_AI"
}
...
# Helper: run dl-init --write-memory with a minimal patch for main branch
_init_repo() {
  local repo="$1"
  local patch
  patch=$(jq -n \
...
@test "dl-sync --version prints Devline version and exits 0" {
  run "$DF_SYNC" --version
  [ "$status" -eq 0 ]
  [[ "$output" =~ Devline ]]
}
...
@test "post-commit: new file gets node in nodes.json with intent" {
  ...
  [ "$status" -eq 0 ]
  node_count=$(jq '.nodes | length' "$REPO/.devline/active/nodes.json")
  [ "$node_count" -gt 1 ]
}
...
TSEOF
  ...
  run bash -c "cd '$REPO' && '$DF_SYNC'"
  [ "$status" -eq 0 ]
  edge_count=$(jq '.edges | length' "$REPO/.devline/active/edges.json")
  [ "$edge_count" -gt 0 ]
}
...
@test "post-commit: deleted file node marked stale:deleted" {
  ...
  run bash -c "cd '$REPO' && '$DF_SYNC'"
  [ "$status" -eq 0 ]
  stale=$(jq -r '.nodes[] | select(.id == "entity:Entities.Comment") | .stale' "$REPO/.devline/active/nodes.json")
  [ "$stale" = "deleted" ]
}
...
@test "post-commit: last_synced updated to HEAD SHA after sync" {
  ...
  run bash -c "cd '$REPO' && '$DF_SYNC'"
  [ "$status" -eq 0 ]
  synced=$(jq -r '.last_synced' "$REPO/.devline/config.json")
  [ "$synced" = "$head_sha" ]
}
...
@test "post-commit: memory.md regenerated after sync" {
  ...
  run bash -c "cd '$REPO' && '$DF_SYNC'"
  [ "$status" -eq 0 ]
  [ -f "$REPO/.devline/active/memory.md" ]
  grep -q "entity" "$REPO/.devline/active/memory.md"
}
...
@test "post-commit: dirty protocol — dirty:true before writes, dirty:false after success" {
  ...
  run bash -c "cd '$REPO' && '$DF_SYNC'"
  [ "$status" -eq 0 ]
  dirty=$(jq -r '.dirty' "$REPO/.devline/config.json")
  [ "$dirty" = "false" ]
}
...
@test "--force --all bypasses max_files_per_sync cap" {
  ...
  local cfg_file="$REPO/.devline/config.json"
  local updated
...
@test "concurrent lock: second dl-sync exits 0 with skip message" {
  _init_repo "$REPO"
  # Create a lock file manually
  mkdir -p "$REPO/.devline"
  echo "99999" > "$REPO/.devline/sync.lock"
  # Run dl-sync; it should detect lock and exit 0
  run bash -c "cd '$REPO' && '$DF_SYNC'"
  [ "$status" -eq 0 ]
...
@test "AI mock mode: DEVLINE_AI_MOCK=1 uses fixture, exit 0" {
  _init_repo "$REPO"
  echo "// change" >> "$REPO/Entities/Comment.cs"
  (cd "$REPO" && git add . && git commit -m "modify Comment" --quiet)
  run bash -c "cd '$REPO' && DEVLINE_AI_MOCK=1 DEVLINE_AI_MOCK_FILE='$FIXTURE_AI' '$DF_SYNC'"
  [ "$status" -eq 0 ]
}
...
@test "manual node preserved: confidence:manual node not overwritten by sync" {
  ...
  local nodes_file="$REPO/.devline/active/nodes.json"
  local updated
  ...
  run bash -c "cd '$REPO' && '$DF_SYNC'"
  [ "$status" -eq 0 ]
  confidence=$(jq -r '.nodes[] | select(.id == "entity:Entities.Comment") | .confidence' "$REPO/.devline/active/nodes.json")
  [ "$confidence" = "manual" ]
}
...
@test "branch-switch: active symlink points to new branch" {
  ...
  [ "$status" -eq 0 ]
  link_target=$(readlink "$REPO/.devline/active")
...
@test "branch-switch: new branch bootstrapped from nearest existing branch" {
  ...
  run bash -c "cd '$REPO' && '$DF_SYNC' --branch-switch"
  [ "$status" -eq 0 ]
  [ -f "$REPO/.devline/active/nodes.json" ]
}
...
@test "branch-switch: stale branch cleanup removes memory for deleted branches" {
  ...
  mkdir -p "$REPO/.devline/branches/ghost-branch"
  echo '{"schema_version":1,"nodes":[]}' > "$REPO/.devline/branches/ghost-branch/nodes.json"
  (cd "$REPO" && git checkout -b cleanup-test --quiet)
  run bash -c "cd '$REPO' && '$DF_SYNC' --branch-switch"
  [ "$status" -eq 0 ]
  [ ! -d "$REPO/.devline/branches/ghost-branch" ]
}

# ─── CI mode ──────────────────────────────────────────────────────────────────

@test "CI mode: no .devline/ directory — exit 0 silently" {
  run bash -c "cd '$REPO' && '$DF_SYNC'"
  [ "$status" -eq 0 ]
}
...
@test "sync: warns when DEVLINE_AI_MOCK is not set" {
  _init_repo "$REPO"
  echo "// change" >> "$REPO/Entities/Comment.cs"
  (cd "$REPO" && git add . && git commit -m "modify Comment" --quiet)
  run bash -c "cd '$REPO' && DEVLINE_AI_MOCK='' '$DF_SYNC' 2>&1"
