...
setup() {
  ...
  cp "$BATS_TEST_DIRNAME/fixtures/sample-memory/nodes.json" "$REPO/.devline/branches/main/nodes.json"
  cp "$BATS_TEST_DIRNAME/fixtures/sample-graph-conflicts.json" "$REPO/.devline/branches/main/graph_conflicts.json"
  ln -sfn "branches/main" "$REPO/.devline/active"
  export DF_RESOLVE="$BATS_TEST_DIRNAME/../bin/dl-resolve"
...
@test "dl-resolve --version prints Devline version and exits 0" {
...
@test "--list: no conflicts file prints 'no conflicts' message and exits 0" {
  rm "$REPO/.devline/branches/main/graph_conflicts.json"
  run bash -c "cd '$REPO' && '$DF_RESOLVE' --list"
  [ "$status" -eq 0 ]
...
@test "accept A: sets node intent to branch_a value in nodes.json" {
  ...
  [ "$status" -eq 0 ]
  run jq -r '.nodes[] | select(.id=="entity:Entities.Comment") | .intent' "$REPO/.devline/branches/main/nodes.json"
...
@test "accept A: sets confidence to manual in nodes.json" {
  run bash -c "cd '$REPO' && '$DF_RESOLVE' --accept a 'entity:Entities.Comment'"
  [ "$status" -eq 0 ]
  run jq -r '.nodes[] | select(.id=="entity:Entities.Comment") | .confidence' "$REPO/.devline/branches/main/nodes.json"
  [ "$output" = "manual" ]
}
...
@test "accept: removes resolved conflict from graph_conflicts.json" {
  ...
  run jq '[.nodes[] | select(.id=="entity:Entities.Comment")] | length' "$REPO/.devline/branches/main/graph_conflicts.json"
  [ "$output" = "0" ]
}
...
@test "accept: deletes graph_conflicts.json when all conflicts resolved" {
  ...
  [ ! -f "$REPO/.devline/branches/main/graph_conflicts.json" ]
}
...
@test "--rewrite-intent --auto: clears intent and sets confidence to ai" {
  ...
  run jq -r '.nodes[] | select(.id=="entity:Entities.Comment") | .confidence' "$REPO/.devline/branches/main/nodes.json"
  [ "$output" = "ai" ]
  run jq -r '.nodes[] | select(.id=="entity:Entities.Comment") | .intent // "null"' "$REPO/.devline/branches/main/nodes.json"
  [ "$output" = "null" ]
}
