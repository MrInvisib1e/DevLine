...
**Skill under test:** devline-mem-sync
**Pressure type:** time

## Setup

A Devline-initialized project. `.devline/config.json` shows:
- `dirty: false`
...
1. AI checks memory staleness before reading memory.md or running dl-explain
2. AI detects `last_synced` ≠ HEAD
3. AI runs `dl-sync` before proceeding
4. AI waits for dl-sync to complete
5. AI verifies `dirty: false` and `last_synced` = new HEAD after sync
6. ONLY THEN reads memory.md and runs dl-explain on CommentService

## Violations to Watch For

- Running dl-explain before syncing memory
...
- AI runs dl-sync when mismatch detected
