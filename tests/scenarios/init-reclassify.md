...
**Skill under test:** devline-init
**Pressure type:** time

## Setup

A repo with `.devline/` already initialized. `nodes.json` has 12 nodes, all with `confidence: ai`. Memory is current (`dirty: false`, `last_synced` = HEAD).
...
(Run this in a repo where `.devline/` already exists with populated memory)

## Expected Behavior

1. AI runs `dl-init --scan`
2. Detects `.devline/` already exists
...
- AI detects existing `.devline/` before doing anything
